...
IMPORTS_AND_SETUP = """import os, sys, copy, time, random, json, shutil
import numpy as np
...
MODEL_AND_CLIENTS = """class BrainTumorDataset(Dataset):
...
def create_data_loaders(pdir="/kaggle/working/data/processed", tdir="/kaggle/working/data/test_set", bs=32, nw=2):
    loaders = {}
    for i in range(1, 4):
        d = f"{pdir}/client{i}"
...
class BrainTumorResNet(nn.Module):
    def __init__(self, n=4):
        super().__init__()
        self.model = models.resnet18(weights=None)  # NO pretrained weights — from scratch
        self.model.fc = nn.Linear(self.model.fc.in_features, n)
    def forward(self, x): return self.model(x)

def create_model(n=4, device="cuda"):
    m = BrainTumorResNet(n).to(device)
    print(f"Model: ResNet-18 (from scratch) | Params: {sum(p.numel() for p in m.parameters()):,} | Classes: {n}"); return m
...
class QPSOServer:
    def __init__(self, gm, clients, device="cuda", beta=0.7):
        self.global_model = gm; self.clients = clients; self.device = device; self.beta = beta
        self.personal_best = {}; self.personal_best_scores = {}
        self.global_best = None; self.global_best_score = 0.0; self.mean_best = None
        print(f"QPSO Server | clients={len(clients)} beta={beta}")
    ...
    def qpso_aggregate(self, cwl):
        ...
        for k in agg:
            ...
            pert = self.beta * torch.abs(self.mean_best[k].float() - agg[k].float()) * torch.log(1.0/u)
            pert = pert.clamp(-0.1, 0.1)
...
TRAINERS = """def train_fedavg(server, clients, gtl, nr=100, le=5, lr=0.001, se=10, sd="/kaggle/working", verbose=False):
...
def train_fedprox(server, clients, gtl, nr=100, le=5, lr=0.001, mu=0.01, se=10, sd="/kaggle/working", verbose=False):
    print("="*80+"\\n  FEDPROX TRAINING START (mu="+str(mu)+")\\n"+"="*80)
    h = {"round":[],"global_test_acc":[],"global_test_loss":[],"client1_val_acc":[],"client2_val_acc":[],"client3_val_acc":[],"round_time":[]}
    best = 0.0
    for rnd in range(1, nr+1):
        t0 = time.time(); print(f"\\n{'='*60} ROUND {rnd}/{nr}"); cw = []
...
def train_qpso(server, clients, gtl, nr=100, le=5, lr=0.001, se=10, sd="/kaggle/working", verbose=False):
    print("="*80+"\\n  QPSO-FL TRAINING START\\n"+"="*80); server.initialize_particles()
    h = {"round":[],"global_test_acc":[],"global_test_loss":[],"global_best_score":[],
    ...
    for rnd in range(1, nr+1):
        ...
        h["global_best_score"].append(server.global_best_score)
        ga, gl = server.evaluate_global_model(gtl); dt = time.time()-t0
        h["round"].append(rnd); h["global_test_acc"].append(ga); h["global_test_loss"].append(gl); h["round_time"].append(dt)
...
EVALUATION = """def plot_accuracy_comparison(dfs, names, save_path):
    n = len(dfs)
...
def generate_confusion_matrix(mp, tl, mname, device="cuda", save_path=None):
    model = BrainTumorResNet(NUM_CLASSES).to(device)
...
def plot_roc_auc(mps, mnames, tl, device="cuda", save_path=None):
    ...
    for ax, mp, mn in zip(axes, mps, mnames):
        ...
        # Handle NaN from poorly trained models
        if np.isnan(yp).any():
...
TRAINING_CELLS = """fedavg_model = create_model(NUM_CLASSES, device)
...
fedavg_history = train_fedavg(fedavg_server, fedavg_clients, data_loaders['global_test'],
    nr=100, le=5, lr=0.001, se=10, verbose=False)"""

FEDPROX_CELL = """torch.cuda.empty_cache()
...
fedprox_history = train_fedprox(fedprox_server, fedprox_clients, data_loaders['global_test'],
    nr=100, le=5, lr=0.001, mu=0.01, se=10, verbose=False)"""

QPSO_CELL = """torch.cuda.empty_cache()
...
EVAL_CELLS = """df_fa = pd.read_csv('/kaggle/working/results/fedavg/metrics.csv')
...
PLOT_CELL = """plot_accuracy_comparison([df_fa, df_fp, df_qp], ['FedAvg','FedProx','QPSO-FL'], '/kaggle/working/results/plots/comparison.png')
...
STATS_CELL = """s1 = statistical_analysis(df_fa, df_qp)
...
SETUP3_DATA_PREP = """# Setup 3: Extreme Label Skew (single class per client)
# Client1 -> Only Glioma, Client2 -> Only Meningioma, Client3 -> Only Pituitary
# No Tumor is ONLY in test set (tests generalization)
...
# Now keep only one class per client
KEEP_CLASS = {1: 0, 2: 1, 3: 3}  # client -> keep class (0=glioma,1=menin,3=pituitary)
...
for cid in range(1, 4):
    ...
    for split in ['train', 'val']:
        X = np.load(f"{d}/X_{split}.npy"); y = np.load(f"{d}/y_{split}.npy")
        keep = KEEP_CLASS[cid]
        mask = y == keep
        X_keep = X[mask]; y_keep = y[mask]
        original_size = len(X)

        # Augment to restore size (train only)
        if split == 'train' and len(X_keep) < original_size:
            extras_X, extras_y = [], []
            need = original_size - len(X_keep)
            for _ in range(need):
                src = X_keep[random.randint(0, len(X_keep)-1)]
                extras_X.append(augment_image(src)); extras_y.append(keep)
            X_keep = np.concatenate([X_keep, np.array(extras_X)])
            y_keep = np.concatenate([y_keep, np.array(extras_y)])

        perm = np.random.permutation(len(X_keep)); X_keep = X_keep[perm]; y_keep = y_keep[perm]
        np.save(f"{d}/X_{split}.npy", X_keep); np.save(f"{d}/y_{split}.npy", y_keep)
        print(f"client{cid} {split}: class={keep} | count={len(X_keep)}")
...
for setup_num, setup_name, setup_desc, data_prep in [
    (1, "setup1_natural", "Setup 1: Natural Heterogeneity (4-class, IID baseline)", SETUP1_DATA_PREP),
    (2, "setup2_label_skew", "Setup 2: Moderate Label Skew (70/10/10/10)", SETUP2_DATA_PREP),
    (3, "setup3_extreme_skew", "Setup 3: Extreme Label Skew (single class per client)", SETUP3_DATA_PREP),
]:
    cells = [
...
All-in-one: Data prep → Training (FedAvg + FedProx + QPSO) → Evaluation"""),
        ("code", IMPORTS_AND_SETUP),
        ("markdown", "## Preprocessor"),
        ("code", PREPROCESSOR),
        ("markdown", "## Model & FL Components"),
        ("code", MODEL_AND_CLIENTS),
        ("markdown", "## Data Preparation"),
        ("code", data_prep),
        ("markdown", "## Training Functions"),
        ("code", TRAINERS),
        ("markdown", "## FedAvg Training\n⚠️ Set `nr=5` for a quick test first!"),
        ("code", TRAINING_CELLS),
    ...
    make_notebook(cells, f"{BASE}/{setup_name}/notebook_setup{setup_num}.ipynb")

# ═══════════════════════════════════════════════════════════════
# PHASE 3: LIGHTWEIGHT MODEL + TUNED QPSO
# ═══════════════════════════════════════════════════════════════

PHASE3_IMPORTS_AND_SETUP = """import os, sys, copy, time, random, json, shutil
import numpy as np
import torch, torch.nn as nn, torch.optim as optim
...
IMG_SIZE = 112        # Reduced from 224 — less spatial info → harder task
LOCAL_EPOCHS = 1      # Reduced from 5 — forces reliance on aggregation
LR = 0.01             # Increased from 0.001 — faster local steps → more client drift
BATCH_SIZE = 64       # Increased from 32 — noisier per-step gradients
NUM_ROUNDS = 100
BETA_START = 1.0      # QPSO β decay: explore broadly early
BETA_END = 0.5        # QPSO β decay: exploit late
FEDPROX_MU = 0.01

print(f"Phase 3 Config: SimpleCNN | img={IMG_SIZE} | E={LOCAL_EPOCHS} | lr={LR} | bs={BATCH_SIZE} | rounds={NUM_ROUNDS}")
print(f"QPSO β: {BETA_START} → {BETA_END} (linear decay) | FedProx μ={FEDPROX_MU}")
...
PHASE3_PREPROCESSOR = """class Preprocessor:
...
PHASE3_MODEL_AND_CLIENTS = """class BrainTumorDataset(Dataset):
    _DEFAULT = transforms.Compose([transforms.ToTensor(),
        transforms.Normalize([0.485,0.456,0.406],[0.229,0.224,0.225])])
    def __init__(self, X, y, transform=None):
        self.X, self.y = X, y; self.transform = transform or self._DEFAULT
...
class SimpleCNN(nn.Module):
    \"\"\"Lightweight 3-layer CNN for Phase 3 experiments.
...
# ═══════════════════════════════════════════════════════════════
class QPSOServer:
    \"\"\"QPSO server with Phase 3 improvements:
...
PHASE3_TRAINERS = """def train_fedavg(server, clients, gtl, nr=None, le=None, lr=None, se=10, sd="/kaggle/working", verbose=False):
...
PHASE3_EVALUATION = """def plot_accuracy_comparison(dfs, names, save_path):
    n = len(dfs)
    fig, axes = plt.subplots(2, max(n,2), figsize=(8*max(n,2), 12))
...
def plot_beta_decay(df_qpso, save_path):
    \"\"\"Phase 3 extra plot: visualize how β decays and its effect on accuracy.\"\"\"
...
PHASE3_TRAINING_CELLS = """fedavg_model = create_model(NUM_CLASSES, device)
fedavg_clients = [FederatedClient(f'client{i}', data_loaders[f'client{i}']['train'],
                  data_loaders[f'client{i}']['val'], device=device) for i in range(1, 4)]
fedavg_server = FedAvgServer(fedavg_model, fedavg_clients, device=device)
fedavg_history = train_fedavg(fedavg_server, fedavg_clients, data_loaders['global_test'],
...
PHASE3_FEDPROX_CELL = """torch.cuda.empty_cache()
fedprox_model = create_model(NUM_CLASSES, device)
fedprox_clients = [FedProxClient(f'client{i}', data_loaders[f'client{i}']['train'],
                   data_loaders[f'client{i}']['val'], device=device) for i in range(1, 4)]
fedprox_server = FedAvgServer(fedprox_model, fedprox_clients, device=device)
...
PHASE3_QPSO_CELL = """torch.cuda.empty_cache()
qpso_model = create_model(NUM_CLASSES, device)
qpso_clients = [FederatedClient(f'client{i}', data_loaders[f'client{i}']['train'],
                data_loaders[f'client{i}']['val'], device=device) for i in range(1, 4)]
...
PHASE3_EVAL_CELLS = """df_fa = pd.read_csv('/kaggle/working/results/fedavg/metrics.csv')
df_fp = pd.read_csv('/kaggle/working/results/fedprox/metrics.csv')
df_qp = pd.read_csv('/kaggle/working/results/qpso/metrics.csv')
comp = build_comparison_df((df_fa,"FedAvg"),(df_fp,"FedProx"),(df_qp,"QPSO-FL"))
print(comp.to_string(index=False))
...
PHASE3_PLOT_CELL = """plot_accuracy_comparison([df_fa, df_fp, df_qp], ['FedAvg','FedProx','QPSO-FL'], '/kaggle/working/results/plots/comparison.png')
for m, n in [('fedavg','FedAvg'),('fedprox','FedProx'),('qpso','QPSO-FL')]:
    generate_confusion_matrix(f'/kaggle/working/models/{m}_best.pth', data_loaders['global_test'], n, device=device, save_path=f'/kaggle/working/results/plots/cm_{m}.png')
plot_client_fairness([df_fa, df_fp, df_qp], ['FedAvg','FedProx','QPSO-FL'], '/kaggle/working/results/plots/fairness.png')
plot_roc_auc(['/kaggle/working/models/fedavg_best.pth','/kaggle/working/models/fedprox_best.pth','/kaggle/working/models/qpso_best.pth'],
...
PHASE3_STATS_CELL = """s1 = statistical_analysis(df_fa, df_qp)
s2 = statistical_analysis(df_fa, df_fp)
print("FedAvg vs QPSO:", json.dumps(s1, indent=2))
print("FedAvg vs FedProx:", json.dumps(s2, indent=2))
summary = {"fedavg": convergence_metrics(df_fa), "fedprox": convergence_metrics(df_fp),
...
# Phase 3 uses identical data prep code, but with IMG_SIZE=112 in Preprocessor
PHASE3_SETUP1_DATA_PREP = SETUP1_DATA_PREP
PHASE3_SETUP2_DATA_PREP = SETUP2_DATA_PREP

# ═══════════════════════════════════════════════════════════════
# GENERATE PHASE 3 NOTEBOOKS
# ═══════════════════════════════════════════════════════════════

for setup_num, setup_name, setup_desc, data_prep in [
    (1, "setup1_natural_lightweight", "Phase 3 / Setup 1: Natural Heterogeneity — SimpleCNN + Dynamic-β QPSO", PHASE3_SETUP1_DATA_PREP),
    (2, "setup2_label_skew_lightweight", "Phase 3 / Setup 2: Moderate Label Skew — SimpleCNN + Dynamic-β QPSO", PHASE3_SETUP2_DATA_PREP),
]:
    cells = [
        ("markdown", f"""# {setup_desc}
**Phase 3 — Lightweight Model Experiment**
- **Model:** SimpleCNN (~120K params) — 100× smaller than ResNet-18
- **Image Size:** 112×112 (reduced from 224×224)
- **Local Epochs:** 1 (reduced from 5) — forces reliance on aggregation
- **Learning Rate:** 0.01 (increased from 0.001) — more client drift
- **QPSO β:** 1.0 → 0.5 (dynamic linear decay, was static 0.7)
- **QPSO perturbation clamp:** ±0.5 (relaxed from ±0.1)
- **QPSO u clamp:** [0.1, 1.0] (relaxed from [0.3, 1.0])

**Hypothesis:** With a capacity-limited model and minimal local training, QPSO's memory-based
exploration (pbest/gbest) should clearly outperform FedAvg's stateless averaging.

**Settings:** GPU P100 · Persistence: Files only · Internet: ON
**Add Inputs:** Brain Tumor MRI Dataset (masoudnickparvar) + BRISC 2025 (briscdataset)"""),
        ("code", PHASE3_IMPORTS_AND_SETUP),
        ("markdown", "## Preprocessor (112×112)"),
        ("code", PHASE3_PREPROCESSOR),
        ("markdown", "## SimpleCNN Model & FL Components"),
        ("code", PHASE3_MODEL_AND_CLIENTS),
        ("markdown", "## Data Preparation"),
        ("code", data_prep),
        ("markdown", "## Phase 3 Training Functions"),
        ("code", PHASE3_TRAINERS),
        ("markdown", "## FedAvg Training (SimpleCNN, E=1, lr=0.01)"),
        ("code", PHASE3_TRAINING_CELLS),
        ("markdown", "## FedProx Training (SimpleCNN, E=1, lr=0.01, μ=0.01)"),
        ("code", PHASE3_FEDPROX_CELL),
        ("markdown", "## QPSO Training (SimpleCNN, Dynamic β: 1.0→0.5)"),
        ("code", PHASE3_QPSO_CELL),
        ("markdown", "## Evaluation & Plots"),
        ("code", PHASE3_EVALUATION),
        ("code", PHASE3_EVAL_CELLS),
        ("code", PHASE3_PLOT_CELL),
        ("markdown", "## Statistical Analysis & Summary"),
        ("code", PHASE3_STATS_CELL),
    ]
    make_notebook(cells, f"{BASE}/{setup_name}/notebook_setup{setup_num}_lightweight.ipynb")
